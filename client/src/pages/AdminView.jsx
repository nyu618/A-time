...
export default function AdminView() {
  ...
  const activeQueues = queues.filter(q => q.status === 'WAITING' || q.status === 'CALLED' || q.status === 'IN_STORE' || q.status === 'ASSESSING');
    ...
    <div className="admin-container">
      <header className="admin-header">
        ...
        <div className="stats" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <span className="stat-badge">待機中: {activeQueues.filter(q => q.status === 'WAITING').length}</span>
                ...
                activeQueues.map((q) => (
                  <div key={q.id} className={`queue-item ${q.status.toLowerCase()}`}>
                    ...
                    <div className="queue-actions">
                      ...
                      {q.status === 'ASSESSING' && (
                        <button className="action-btn arrive" onClick={() => handleAction(q.id, 'complete')} title="査定完了">
                          <UserCheck size={18} />
                          <span>査定完了</span>
                        </button>
                      )}
